...
from hirehuntpilot.apply import chrome, dashboard, prompt as prompt_mod
from hirehuntpilot.apply.chrome import (
    launch_chrome, cleanup_worker, kill_all_chrome,
    reset_worker_dir, cleanup_on_exit, _kill_process_tree,
    BASE_CDP_PORT,
)
from hirehuntpilot.apply.dashboard import (
...
def _load_blocked():
    from hirehuntpilot.config import load_blocked_sites
...
def acquire_job(target_url: str | None = None, min_score: int = 7,
    ...
    try:
        ...
        from hirehuntpilot.config import is_manual_ats
...
def run_job(job: dict, port: int, worker_id: int = 0,
            model: str = "default", dry_run: bool = False) -> tuple[str, int]:
    """Run BrowserApplyAgent to apply for one job.

    Returns:
        Tuple of (status_string, duration_ms).
    """
    from hirehuntpilot import config
    resume_data = config.load_profile()
    resume_pdf_path = config.RESUME_PDF_PATH if config.RESUME_PDF_PATH.exists() else None

    # Load active model config name for apply_agent role
    model_config_name = config.get_agent_model("apply_agent")
    ...
    from hirehuntpilot.apply.agent import BrowserApplyAgent
    ...
    try:
        with sync_playwright() as p:
            ...
            # Set up Telegram alert function if token is configured
            telegram_alert_fn = None
            token = os.environ.get("TELEGRAM_BOT_TOKEN", "")
            chat_id_str = os.environ.get("TELEGRAM_CHAT_ID", "")
            if token and chat_id_str:
                async def _send_tg_alert(message: str, screenshot_b64: str | None = None):
                    from telegram import Bot
                    bot = Bot(token)
                    chat_id = int(chat_id_str)
                    await bot.send_message(chat_id=chat_id, text=f"⚠️ [W{worker_id}] {message}")
                    if screenshot_b64:
                        import base64
                        img_bytes = base64.b64decode(screenshot_b64)
                        await bot.send_photo(chat_id=chat_id, photo=img_bytes)
                    return "captcha_escalated"
                
                import asyncio
                def telegram_alert_fn(message: str, screenshot_b64: str | None = None):
                    try:
                        return asyncio.run(_send_tg_alert(message, screenshot_b64))
                    except Exception as e:
                        logger.warning("Telegram alert failed: %s", e)
                        return "error"

            # Instantiate and run the agent
            agent = BrowserApplyAgent(
                page=page,
                model_config_name=model_config_name,
                resume_data=resume_data,
                job_data=job,
                resume_pdf_path=resume_pdf_path,
                telegram_alert_fn=telegram_alert_fn,
            )

            # Navigate to the job URL to start the application
            ...
            if status == "success":
                ...
                add_event(f"[W{worker_id}] TIMEOUT/SKIPPED ({elapsed}s)")
                update_state(worker_id, status="failed", last_action=f"skipped ({elapsed}s)")
