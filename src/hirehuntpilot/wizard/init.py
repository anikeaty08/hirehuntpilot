...
def _setup_ai_features() -> None:
    ...
    env_lines = ["# hirehuntpilot configuration", ""]
...
def _setup_auto_apply() -> None:
    ...
    console.print(Panel(
        ...
        "hirehuntpilot can autonomously fill and submit job applications\n"
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        if ENV_PATH.exists():
            ...
            ENV_PATH.write_text(f"# hirehuntpilot configuration\nCAPSOLVER_API_KEY={capsolver_key}\n", encoding="utf-8")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]hirehuntpilot Setup Wizard[/bold green]\n\n"
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]hirehuntpilot init[/bold]).[/dim]"
    elif tier == 2:
