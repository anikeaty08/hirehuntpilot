...
Usage (via CLI):
    hirehuntpilot run                        # all stages, sequential
    hirehuntpilot run --stream               # all stages, concurrent
    hirehuntpilot run discover enrich        # specific stages
    hirehuntpilot run score tailor cover     # LLM-only stages
    hirehuntpilot run --dry-run              # preview without executing
"""
...
STAGE_META: dict[str, dict] = {
    "discover": {"desc": "Job discovery via hirehunt sources"},
...
def _run_discover(workers: int = 1) -> dict:
    """Stage: Job discovery via the hirehunt framework."""
    del workers
    console.print("  [cyan]hirehunt source crawl...[/cyan]")
    try:
        from hirehuntpilot.discovery.hirehunt import run_discovery

        return run_discovery()
    except Exception as e:
        log.error("hirehunt discovery failed: %s", e)
        console.print(f"  [red]hirehunt error:[/red] {e}")
...
def _run_cover(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from hirehuntpilot.scoring.cover_letter import run_cover_letters
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]hirehuntpilot Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
