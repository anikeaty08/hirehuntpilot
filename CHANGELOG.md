...
All notable changes to HireHuntPilot will be documented in this file.
...
### Added
- **Parallel workers for discovery/enrichment** - `hirehuntpilot run --workers N` enables
...
- **Apply utility modes** - `--gen` (generate prompt for manual debugging), `--mark-applied`,
  `--mark-failed`, `--reset-failed` flags on `hirehuntpilot apply`
- **Dry-run mode** - `hirehuntpilot apply --dry-run` fills forms without clicking Submit
...
- **Config YAML not found after install** - moved `config/` into the package at
  `src/hirehuntpilot/config/` so YAML files (employers, sites, searches) ship with `pip install`
...
- Interactive setup wizard (`hirehuntpilot init`)
